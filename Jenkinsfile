    ...
    stages {
        ...
        stage('Deploy for production') {
            steps {
                ...
                script {
                sh '''
                sed -i "s/\(image: julienvb\/labo:datascientest-project-movie-service\)/\1-prod/" values.yml
                sed -i "s/\(image: julienvb\/labo:datascientest-project-cast-service\)/\1-prod/" values.yml
